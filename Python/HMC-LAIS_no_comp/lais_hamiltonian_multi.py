...
class Lais_Ham():
    ...
    def Phi(self,x,chains,m,den,n=None):
    ...
    def weighting(self,samples,new_samples,den="temporal"):
        if len(new_samples.shape)==2:
            ...
            for t in range(1,new_samples.shape[0]):
                p = self.Phi(new_samples[t,:],samples,t//k,den)
        ...
        if len(new_samples.shape) == 3:
            ...
            for n in range(new_samples.shape[0]):
                for t in range(1,new_samples.shape[1]):
                    p = self.Phi(new_samples[n,t,:],samples,t//k,den,n)
...
def plot_(function,xlims,ylims,log=True):
    fig = plt.figure()
    ax = fig.add_subplot(1,1,1)
...
# plot_(logpdf_mix_multivariate,(-9,5),(-5,9))
