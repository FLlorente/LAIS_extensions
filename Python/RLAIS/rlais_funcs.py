...
def met_hast_multi(target,n_samples,initial_point,cov):
    ...
    values = zeros((n_samples,len(initial_point)))
    z_all = zeros((n_samples,len(initial_point)))
...
def Phi(x,chains,cov,m):
    """Denominator of LAIS
    I think this is the spatial denominator"""
    sum = 0
    m=int(m)
    N = chains.shape[0]
...
def weighting(target,chains,samples,cov):
    ...
    # print(chains.shape,samples.shape)
    # print(k)
